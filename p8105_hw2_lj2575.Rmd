...
Now we provide a description for the separate data sets and the final resulted joined data set. The pols_month data set contains `r ncol(pols_month)` variables, including year, month, the number of GOP/Dem governors, senators, and representatives respectively, and if the president of the particular month is Democrat or Republican. On the other hand, the snp data set contains only `r ncol(snp)` variables; namely, year, month, and the closing values of the S&P stock index on some day during the given year and month. Finally, the unemployment data set contains also `r ncol(unemployment)` variables: year, month, and the percentage of unemployment date of a given year and month. At last, the result data set from joining the three data sets, pols_data, contains `r ncol(pols_data)` variables and `r nrow(pols_data` observations. Furthermore, the range is between `r min(pols_data$year)` and `r max(pols_data$year)` with a difference of `r max(pols_data$year) - min(pols_data$year)` years. 